...
def load_images():
    global sun_image, cloud_image, fumes_image
    ...
    fumes_image = cv2.imread('fumes.png', cv2.IMREAD_UNCHANGED)
    if fumes_image is None:
        print("Warning: fumes.png not found. Using default fumes.")
...
def apply_angry_filter(frame, faces):
    global fumes_image
    
    # Create a red tint
    red_tint = np.full(frame.shape, (0, 0, 130), dtype=np.uint8)
    cv2.addWeighted(red_tint, 0.2, frame, 0.8, 0, frame)
    
    for (x, y, w, h) in faces:
        if fumes_image is not None:
            # Resize fumes image
            fumes_width = w
            fumes_height = h // 2
            resized_fumes = cv2.resize(fumes_image, (fumes_width, fumes_height))
            
            # Calculate position (above the face)
            fumes_y = max(0, y - fumes_height)
            fumes_x = x
            
            # Get the alpha channel of the fumes image
            if resized_fumes.shape[2] == 4:
                alpha_fumes = resized_fumes[:, :, 3] / 255.0
                fumes_rgb = resized_fumes[:, :, :3]
            else:
                alpha_fumes = np.ones((fumes_height, fumes_width))
                fumes_rgb = resized_fumes
            
            # Blend the fumes image with the frame
            for c in range(0, 3):
                frame[fumes_y:fumes_y+fumes_height, fumes_x:fumes_x+fumes_width, c] = \
                    (alpha_fumes * fumes_rgb[:, :, c] + 
                     (1 - alpha_fumes) * frame[fumes_y:fumes_y+fumes_height, fumes_x:fumes_x+fumes_width, c])
        else:
            # Fallback to drawing simple fumes
            fume_y = max(0, y - h // 2)
            fume_height = min(h // 2, y)
            for i in range(3):
                fume_x = x + (i * w // 3)
                cv2.ellipse(frame, (fume_x, fume_y), (w // 6, fume_height // 2), 
                            0, 0, 180, (200, 200, 200), -1)
...
def apply_filter(frame, emotion, rain_drops, faces):
    ...
    elif smoothed_emotion == 'angry':
        return apply_angry_filter(frame, faces)
    else:
